...
`));
  ...
  processTemplate(content) {
    ...
    const replacements = {
      ...
      'EVENT_SYSTEM': this.config.event_system || this.config.events,
      'EVENTS': this.config.events || this.config.event_system,
      'RESOURCE': this.config.default_resource,
      'TIMEOUT': this.config.http_timeout_ms?.toString(),
      'PORT': this.config.port,
      'ENABLE_OPENAPI': this.config.enable_openapi,
      'ENABLE_IDEMPOTENCY': this.config.enable_idempotency,
      'ENABLE_RATE_LIMIT': this.config.enable_rate_limit,
      'ENABLE_TRACING': this.config.enable_tracing
    };
